...
@section('content')
    <div class="col-lg-9 col-sm-12">
        ...
        <h3>Create record, when was last service for '{{$reminder->name}}'</h3>
        <form action="{{ url('car/'.$car->id . '/reminder/'.$reminder->id.'/step/3') }}" method="POST" class="form-horizontal">
            {{ csrf_field() }}
            <div class="row">
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input required="" type="text" class="form-control" id="name" name="name"
                               placeholder="Record name" value="{{$reminder->name}}" >
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <h3>If you don't want to create new record, specify when new '{{$reminder->name}}' happens:</h3>
        <form action="{{ url('car/'.$car->id . '/reminder/'.$reminder->id.'/step/4') }}" method="POST" class="form-horizontal">
            {{ csrf_field() }}
            <div class="row">
                <div class="form-group col-sm-6">
                    <label class="control-label col-sm-4" for="next_service_date">Date:</label>
                    <div class="col-sm-8">
                        <input type="date" class="form-control" id="next_service_date" name="next_service_date"
                               placeholder="Next Service Date" value="0">
                    </div>
                </div>
                <div class="form-group col-sm-6">
                    <label class="control-label col-sm-4" for="next_service_mileage">Mileage:</label>
                    <div class="col-sm-8">
                        <input type="number" step="0.01" class="form-control" id="next_service_mileage" name="next_service_mileage"
                               placeholder="Next Service Mileage" value="0">
                    </div>
                </div>
            </div>
